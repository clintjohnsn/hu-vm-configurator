...
import { MockDataType, setFunctionType, StateType, SummaryType } from '../Types';
...
const Summary:React.FC<Props> = ({data,state,setFunctions})=>{
    ...
    summaryData.bandwidth = state.bandwidth?getMemoryFormat(state.bandwidth):undefined;
    summaryData.instance = {
        name: data.instances.find(instance=>instance.id === state.instance?.instanceId)?.desc,
        cpuVariant:state.instance?.cpuVariant,
        memoryVariant:state.instance?.memoryVariant,
    }
    summaryData.storage = {
        root:{...state.storage.root,type:data.storage.find(s=>s.id === state.storage.root.typeId)?.name},
        ext:[...state.storage.ext].map(storage=>{
            return {...storage,type:data.storage.find(s=>s.id === storage.typeId)?.name}
        }),
    }
    console.log(summaryData)
    var exportData = "text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(summaryData));
    ...
    return(
        <div>
            <Button href={`data:${exportData}`} download={"configuration.json"} variant="contained">Generate JSON</Button>
            {image?
                <div>
                    <h2>Image</h2>
                    <ImageCard image={image} state={state} setFunctions={setFunctions}/>
                </div>
            :null}
